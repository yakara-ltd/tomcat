...
* `node["tomcat"]["certificate_dn"]` - DN for the certificate, default `cn=localhost`.
* `node["tomcat"]["certificate_alias"]` - alias for the certificate
* `node["tomcat"]["keytool"]` - path to keytool, used for generating the certificate, location varies by platform
* `node["tomcat"]["key_algorithm"]` - Key algorithm used when generating private key, default `RSA`.
* `node["tomcat"]["key_size"]` - Key size used when generating private key, default `2048`.
* `node["tomcat"]["key_signature_algorithm"]` - Key signature algorithm used when generating private key, default `SHA256withRSA`.
